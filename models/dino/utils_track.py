...
def bbox_overlaps(boxes, query_boxes):
    ...
    if isinstance(boxes, np.ndarray):
        ...
        out_fn = lambda x: x
    boxes_iou = deepcopy(boxes)
    boxes_iou[:, 0] -= boxes_iou[:, 2]/2
    boxes_iou[:, 1] -= boxes_iou[:, 3]/2
    boxes_iou[:, 2] += boxes_iou[:, 0]
    boxes_iou[:, 3] += boxes_iou[:, 1]

    query_boxes_iou = deepcopy(query_boxes)
    query_boxes_iou[:, 0] -= query_boxes_iou[:, 2]/2
    query_boxes_iou[:, 1] -= query_boxes_iou[:, 3]/2
    query_boxes_iou[:, 2] += query_boxes_iou[:, 0]
    query_boxes_iou[:, 3] += query_boxes_iou[:, 1]
    
    box_areas = (boxes_iou[:, 2] - boxes_iou[:, 0] + 1) * (boxes_iou[:, 3] - boxes_iou[:, 1] + 1)
    query_areas = (query_boxes_iou[:, 2] - query_boxes_iou[:, 0] + 1) * (query_boxes_iou[:, 3] - query_boxes_iou[:, 1] + 1)

    iw = (torch.min(boxes_iou[:, 2:3], query_boxes_iou[:, 2:3].t()) - torch.max(boxes_iou[:, 0:1],
                                                                        query_boxes_iou[:, 0:1].t()) + 1).clamp(min=0)
    ih = (torch.min(boxes_iou[:, 3:4], query_boxes_iou[:, 3:4].t()) - torch.max(boxes_iou[:, 1:2],
                                                                        query_boxes_iou[:, 1:2].t()) + 1).clamp(min=0)
