...
class DeformableTransformer(nn.Module):
    ...
    def init_ref_points(self, use_num_queries):
        # self.refpoint_embed_xy = nn.Embedding(use_num_queries, 2)
        # self.refpoint_embed_wh = nn.Embedding(use_num_queries, 2)
        if self.random_refpoints_xy:
            # import pdb; pdb.set_trace()
            self.refpoint_embed_xy = nn.Embedding(use_num_queries, 2)
            self.refpoint_embed_wh = nn.Embedding(use_num_queries, 2)
            self.refpoint_embed_xy.weight.data.uniform_(0, 1)
            self.refpoint_embed_xy.weight.data = inverse_sigmoid(self.refpoint_embed_xy.weight.data)
            self.refpoint_embed_xy.weight.requires_grad = False
        else:
    ...
    def init_tgt_embed_with_attpool(self, use_num_queries, template_features):
        for i, feature in enumerate(template_features):
            # print(feature.shape)
            feat = self.attnpool(feature)
            feat = feat[:, None, :].repeat(1, use_num_queries, 1)
            # print(feat.shape)
            if i == 0:
                target = feat
            else:
